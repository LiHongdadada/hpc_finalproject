...
//	VecView(G_q ,PETSC_VIEWER_STDOUT_WORLD);
	...
	VecSet(T, 0.0);
            for (int i = 0; i < n+1; i++)
        {
                VecSetValues(T, 1, &i, &one, INSERT_VALUES);
        }
        for (int i = n+1; i < n * (n + 1) + 1; i += n + 1)
        {
                VecSetValues(T, 1, &i, &one, INSERT_VALUES);
...
=======
	...
	//        VecView(G_q ,PETSC_VIEWER_STDOUT_WORLD);
        ...
        if ((iter%10)==0)
        {
            ...
            PetscViewerCreate(PETSC_COMM_WORLD,&h5);
            PetscViewerHDF5Open(PETSC_COMM_WORLD,"implicit.h5", FILE_MODE_WRITE, &h5);
            PetscObjectSetName((PetscObject) T, "implicit-vector");
            PetscObjectSetName((PetscObject) tem, "implicit-necess-data");
            VecView(tem, h5);
            VecView(T, h5);
            PetscViewerDestroy(&h5);
        }
